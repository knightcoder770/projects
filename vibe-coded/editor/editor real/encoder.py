...
class EncoderEngine:
    ...
    def encode_from_avi(self,
        ...
        """
        Encode a disk-based AVI file + audio WAV into final MP4.
        This is the new main encoding path (disk-based, no RAM limit).

        Uses FFmpeg to:
        1. Take the AVI file (raw frames written by capture engine)
        2. Mix with WAV audio (trimmed to exact video duration)
        3. Encode with NVENC (GPU) or libx264 (CPU fallback)
        4. Output final MP4/MKV/GIF
        """
        if not os.path.exists(avi_path):
            return {"success": False, "message": "AVI file not found"}
        ...
        try:
            ...
            if not cap.isOpened():
                return {"success": False, "message": "Cannot open AVI file"}
            ...
            # Use actual fps from AVI (most accurate)
            use_fps = actual_fps if actual_fps > 0 else fps
            use_fps = round(use_fps, 3)
            ...
            print(f"[EncoderEngine] encode_from_avi | "
                  f"{width}x{height} @ {use_fps}fps | "
                  f"format:{format} | quality:{quality}")

            # ── Build FFmpeg command ──
            # Input: AVI file (no pipe needed — file on disk)
            ffmpeg_quality = {
                "low":    ("28", "fast"),
                "medium": ("23", "fast"),
                "high":   ("18", "medium"),
                "max":    ("15", "slow"),
            }.get(quality, ("18", "medium"))

            crf, preset = ffmpeg_quality

            # Even dimensions required by H.264
            w2 = width  if width  % 2 == 0 else width  - 1
            h2 = height if height % 2 == 0 else height - 1
            scale_filter = f"scale={w2}:{h2}" if (w2 != width or h2 != height) else None
            ...
            if format == "gif":
                gif_cmd = [
                    self._ffmpeg_path, "-y",
                    "-i", avi_path,
                    "-vf", f"fps=15,scale={w2}:-1:flags=lanczos,split[s0][s1];"
                           "[s0]palettegen[p];[s1][p]paletteuse",
                    "-loop", "0",
                    output_path
                ]
                r = subprocess.run(gif_cmd, capture_output=True, timeout=600)
                if r.returncode == 0:
                    return self._build_result(output_path, total_frames, use_fps)
                return {"success": False, "message": "GIF encoding failed"}

            cmd.append(output_path)

            print(f"[EncoderEngine] Running: {' '.join(cmd[:6])}...")
            r = subprocess.run(cmd, capture_output=True, timeout=3600)

            if r.returncode != 0:
                err = r.stderr.decode('utf-8', errors='replace')
                # NVENC failed — retry with CPU
                if self._nvenc_h264 and ('nvenc' in err.lower() or 'nv' in err.lower()):
                    print("[EncoderEngine] NVENC failed, retrying with CPU...")
                    cmd2 = [self._ffmpeg_path, "-y", "-i", avi_path]
                    if audio_path and os.path.exists(audio_path):
                        cmd2 += ["-i", audio_path,
                                 "-map", "0:v:0", "-map", "1:a:0", "-shortest"]
                    if scale_filter:
                        cmd2 += ["-vf", scale_filter]
                    cmd2 += ["-c:v", "libx264", "-preset", preset, "-crf", crf,
                    ...
                    r2 = subprocess.run(cmd2, capture_output=True, timeout=3600)
                    if r2.returncode == 0:
                        return self._build_result(output_path, total_frames, use_fps)
                    err = r2.stderr.decode('utf-8', errors='replace')
                return {"success": False, "message": f"Encoding failed: {err[-300:]}"}

            return self._build_result(output_path, total_frames, use_fps)

        except Exception as e:
            print(f"[EncoderEngine] encode_from_avi error: {e}")
            return {"success": False, "message": str(e)}
        finally:
            self._is_encoding = False

    def _build_result(self, output_path, total_frames, fps) -> dict:
        """Build success result dict with file metadata."""
        try:
            ...
            print(f"[EncoderEngine] Done | {output_path} | "
                  f"{size_mb}MB | {duration}s")
            return {
                "success":    True,
